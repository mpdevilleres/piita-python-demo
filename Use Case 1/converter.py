...
def create_output_file():
    ...
    with open(os.path.join(base_dir, 'output.csv'), 'w', newline='') as f:
        writer = csv.DictWriter(f, header)
        writer.writeheader()


def append_to_output(data=None):
    with open(os.path.join(base_dir, 'output.csv'), 'a', newline='') as f:
        writer = csv.DictWriter(f, header)
...
def main():
    prep()
    create_output_file()
    text = get_text_from_file(filename='retired_asset.txt')

    # SPLITS TEXT IN TO INDIVIDUAL FILES
    split_word = 'SAMPLE COMPANY'
